...
void main() {
  group('Trip Lifecycle Integration Tests', () {
    ...
    test(
      '1. Confirmed EXIT starts IN_PROGRESS trip, Confirmed ENTRY completes trip',
      () async {
    ...
    test(
      '2. Duplicate EXIT and Duplicate ENTRY do not create duplicate trips or completions',
      () async {
        final exitEvt = GeofenceEvent(
          id: 'evt_exit_EV-101_Warehouse_p1',
          vehicleId: 'EV-101',
    ...
    test(
      '3. Late EXIT revises start boundary without duplicate trip creation',
      () async {
        ...
        expect(
          trips.first.startTime.toUtc().toIso8601String(),
          lateExitTime.toUtc().toIso8601String(),
        );
      },
    );

    test(
      '3b. Late ENTRY revises completion boundary without creating duplicate trip',
      () async {
        ...
        expect(
          trips.first.endTime?.toUtc().toIso8601String(),
          lateEntryTime.toUtc().toIso8601String(),
        );
      },
    );

    test(
      '4. Trip metrics calculation (Distance, Max Speed, SOC used)',
      () async {
    ...
    test(
      '5. Trip state survives application restart / fresh instance query',
      () async {
        final exitEvt = GeofenceEvent(
        ...
        final freshTripRepo = TripRepositoryImpl(
          localDataSource: freshTripDS,
          eventBus: eventBus,
        );

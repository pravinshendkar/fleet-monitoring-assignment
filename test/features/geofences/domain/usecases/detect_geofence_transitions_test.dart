...
void main() {
  group('DetectGeofenceTransitionsUseCase Targeted Verification Tests', () {
    ...
    test('1. Out-of-order location events are sorted by eventTimestamp before transition evaluation', () async {
      ...
      final events = await useCase(DetectGeofenceTransitionsParams(
      ...
      expect(events.first.eventTimestamp, pEarly2.eventTimestamp); // Entry confirmed at second reading
    });

    test('2. Late location event is inserted into sequence deterministically without duplicate transitions', () async {
      ...
      final events1 = await useCase(DetectGeofenceTransitionsParams(
        packets: [p1, p2],
        activeVehicleGeofences: {},
      ));
      ...
      final events2 = await useCase(DetectGeofenceTransitionsParams(
        packets: [p1, pLate, p2],
        activeVehicleGeofences: {},
      ));
    ...
    test('3. Duplicate packets produce identical deterministic transition identity', () async {
      final p1 = TelemetryPacket(
        packetId: 'p1',
        vehicleId: 'v1',
      ...
      final eventsA = await useCase(DetectGeofenceTransitionsParams(packets: [p1, p2]));
      final eventsB = await useCase(DetectGeofenceTransitionsParams(packets: [p1, p2]));
    ...
    test('4. Missing intervals do not invent fake intermediate transitions', () async {
    ...
    test('5. Deactivated geofence generates zero transitions while historical data is preserved', () async {
